...
class TranscriptionExporter:
    """
    Exporta resultados de transcrição para diferentes formatos.
    Recebe o dict retornado por TranscriptionService.transcribe().
    """
    ...
    def save_all(
        ...
        Returns:
            {
                "success": bool,          # True se ao menos um exportou
                "results": {
    ...
    def _resolve_stem(result: Dict[str, Any], file_name: Optional[str]) -> str:
        ...
        return Path(original).stem  # "aula_sample.mp4" → "aula_sample"